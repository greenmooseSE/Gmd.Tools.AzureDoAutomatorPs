<#
.SYNOPSIS
Set the acceptance criteria of an Azure DevOps work item

.DESCRIPTION
Updates the acceptance criteria field of an existing work item (typically used for Stories).
...
The new acceptance criteria text (required)
...
.EXAMPLE
    $updated = .\Set-AzDoAcceptanceCriteria.ps1 -Organization "myorg" -Project "myproject" -WorkItemId 123 -AcceptanceCriteria "Must support OAuth 2.0"
...
if ([string]::IsNullOrWhiteSpace($AcceptanceCriteria)) {
    Write-Error "Parameter 'AcceptanceCriteria' cannot be empty."
}

$null = & ssLogIt.ps1 -Level Info -Message "Updating acceptance criteria for work item (ID: $WorkItemId)"
...
try {
    $updateFields = @{
        $script:FIELD_ACCEPTANCE_CRITERIA = $AcceptanceCriteria
    }
    ...
    $null = & ssLogIt.ps1 -Level Info -Message "Successfully updated acceptance criteria for work item (ID: $($updated.id))"

    return $updated
}
catch {
    $null = & ssLogIt.ps1 -Level Error -Message "Failed to update acceptance criteria: $_" -Exception $_
    Write-Error $_
    throw
}
