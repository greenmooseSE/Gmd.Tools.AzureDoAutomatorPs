...
if ($PSBoundParameters.ContainsKey('Fields') -and $null -ne $Fields) {
    Assert-FieldsNotReadOnly -Organization $Organization -Project $Project -WorkItemType $script:WORKITEM_TYPE_STORY -Fields $Fields
}
if ($PSBoundParameters.ContainsKey('State')) {
    Assert-StateIsWritable -Organization $Organization -Project $Project -WorkItemType $script:WORKITEM_TYPE_STORY -State $State
}
    ...
    if ($PSBoundParameters.ContainsKey('Id')) {
        ...
        if ($null -ne $existingStory) {
            ...
            if ($updateFields.Count -eq 0) {
                Write-Error "At least one field must be provided for update (Title, Description, AcceptanceCriteria, AcScenarios, ExtraInformation, StoryPoints, Priority, OriginalEstimate, FixedIn, DeployedToDev, DeployedToStaging, DeployedToProduction, State, or -Fields)."
            }
