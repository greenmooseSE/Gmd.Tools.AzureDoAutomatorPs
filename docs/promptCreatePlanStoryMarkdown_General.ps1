<#
.SYNOPSIS
    Generates a prompt for adding Story work items to an existing feature plan.

.DESCRIPTION
    Produces a dynamically composed prompt for AI agents to create one or more `### Story:`
    items within an existing (or new) feature plan file, merging the plan creation rules,
    general story rules, project-specific story rules, and architectural rules from their
...
.PARAMETER FeatureId
    The Azure DevOps work item ID of the parent feature under which the story/stories
...
.EXAMPLE
    .\docs\promptCreatePlanStoryMarkdown_General.ps1 -EpicId 1305 -FeatureId 1590 -OutputToClipboard

.EXAMPLE
    .\docs\promptCreatePlanStoryMarkdown_General.ps1 -EpicId 1305 -FeatureId 1590 -OutputFile "tmp\story-prompt.md"
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
$template          = hReadInclude 'createPlanStoryPromptTemplate.md'
...
$azdoConfig = @(
    ...
    "- **Epic ID**: AB#$EpicId",
    "- **Feature ID**: AB#$FeatureId (story to be added within this existing feature)",
...
$prompt = $prompt.Replace('{{GENERATED_BY}}', 'promptCreatePlanStoryMarkdown_General.ps1')
